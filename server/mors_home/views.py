...
aggregator = AggregateSearch(LdaEngine.from_configfile(), TfidfEngine.from_configfile(), D2VEngine.from_configfile())
    ...
    return Response(
        {
            ...
            "result": [
                url_d
                for url_d in Site.objects.mongo_find(
                    {"url": {"$in": [r[0] for r in result]}}, {"url": 1, "tittle": 1, "description": 1, "_id": 0}
                )
            ],
        }
    )
