...
class LDA(object):
    def __init__(self, max_workers, num_topics, passes):
        self.passes = passes
    ...
    def train(self, preprocessed_docs):
        ...
        logger.info(
            "Built corpus. Starting actual training with " "%d topics, %d workers, %d passes",
            self.num_topics,
            self.max_workers,
            self.passes,
        )
        self.model = LdaMulticore(
            corpus, num_topics=self.num_topics, id2word=self.dictionary, workers=self.max_workers, passes=self.passes
        )
    ...
    def with_url_handling(max_workers, num_topics, passes):
        return LDA(max_workers, num_topics, passes)
