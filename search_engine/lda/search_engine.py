...
class LdaEngine(object):
    ...
    def save_index(self, index_path, url_path):
        ...
        logger.info("Saving urls to %s", url_path)
    ...
    def load_index(self, index_path, url_path):
        ...
        logger.info("Loading urls from %s", url_path)
        self.urls = load(url_path)

    def infer(self, document):
        logger.info('Infering document "%s"', document[:20] + "..." if len(document) > 20 else document[:20])
        text = self.preprocessor.preprocess_doc(document)
        bow = self.dictionary.doc2bow(text)
        return self.lda_model[bow]

    def infer_all(self, docs_with_urls):
        logger.info("Infering %d documents ", len(docs_with_urls))
    ...
    def create_index(self, docs_with_urls):
        logger.info("Creating index out of %d documents", len(docs_with_urls))
    ...
    def search(self, query):
        logger.info("Searching for documents similar to %s", query)
    ...
    def adjdust(self, query_length, similarity):
    ...
    def with_loaded_model():
        ...
        search_engine = LdaEngine(config['topics'])

        search_engine.load_model(config['model_path'], config['dict_path'])

        search_engine.load_index(config['index_path'], config['url_path'])
